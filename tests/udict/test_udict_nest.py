...
class TestClass:
    ...
    def test_udict_nest_case01(self):
        data = [
        ...
        result = uDict().nest(data, "id", "parent_id", "children")
        assert result == expect

    def test_udict_nest_case02(self):
        data = [
            {"id": 1, "parent_id": None, "name": "John"},
            {"id": 2, "parent_id": 1, "name": "Frank"},
            {"id": 3, "parent_id": 2, "name": "Tony"},
        ...
        with pytest.raises(ValueError) as e:
            result = uDict().nest(data, "id", "id", "children")
        assert str(e.value) == ''

        with pytest.raises(ValueError) as e:
            result = uDict().nest(data, "id", "parent_id", "id")
        assert str(e.value) == ''

        with pytest.raises(ValueError) as e:
            result = uDict().nest(data, "id", "parent_id", "parent_id")
        assert str(e.value) == ''
    ...
    def test_udict_nest_case04(self):
        data = [
            [{"id": 1, "parent_id": None, "name": "John"}],
            [{"id": 2, "parent_id": 1, "name": "Frank"}],
        ]
